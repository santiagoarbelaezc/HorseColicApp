<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=120&section=header&animation=fadeIn" />
</div>

<h1 align="center">🐎 EquineHealth AI Pro</h1>

<h3 align="center">🚀 Diagnóstico Inteligente de Cólico Equino con Inteligencia Artificial Dual</h3>

<p align="center">
  Plataforma PWA de alta precisión diseñada para veterinarios. <br>
  Combina modelos de Machine Learning (V7) para pronóstico y LLaMA 3.1 para estrategias clínicas.
</p>

---


## 📋 **PENDIENTE IMPLEMENTACION DEL CHAT CON EL AGENTE**
- graficos de overfiting con redes neurales y arboles

---

## 📋 **Descripción del Proyecto**

**EquineHealth AI Pro** es una herramienta de soporte de decisiones clínicas enfocada en la detección temprana y el pronóstico del cólico equino. Utiliza un enfoque híbrido de **Inteligencia Artificial Dual**: un motor estadístico de precisión (Random Forest/Stacking) para predecir la supervivencia y un motor de razonamiento (LLaMA 3.1 via Groq) para recomendar planes de acción quirúrgicos o médicos.

> ⚠️ **Estado del Proyecto:** Producción / Estable (V9)
> Optimizado para despliegue en Vercel con soporte PWA completo.

---

## 🏗️ **Dashboard Estratégico Dual-Engine**

El corazón de la aplicación es un panel de resultados que separa el **Pronóstico Estadístico** de la **Estrategia Médica**:
- **Prognosis Engine (V7)**: Predice si el equino Vive, Muere o requiere Eutanasia con un Accuracy del 66%+.
- **Strategic AI Agent**: Genera informes detallados de 4+ párrafos sobre la fisiopatología del caso, recomendando Cirugía o Tratamiento Médico basado en constantes vitales.

---

## 🔧 **Stack Tecnológico**

### **Core & Frontend (PWA)**
<div align="center">
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" />
  <img width="8" />
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white" />
  <img width="8" />
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white" />
  <img width="8" />
  <img src="https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white" />
</div>

### **Backend & API**
<div align="center">
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
  <img width="8" />
  <img src="https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white" />
  <img width="8" />
  <img src="https://img.shields.io/badge/Uvicorn-4051B5?style=for-the-badge&logo=uvicorn&logoColor=white" />
</div>

### **Data Science & AI**
<div align="center">
  <img src="https://img.shields.io/badge/Scikit--Learn-F7931E?style=for-the-badge&logo=scikit-learn&logoColor=white" />
  <img width="8" />
  <img src="https://img.shields.io/badge/Groq-000000?style=for-the-badge&logo=groq&logoColor=white" />
  <img width="8" />
  <img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white" />
</div>

### **Hosting & Deployment**
<div align="center">
  <img src="https://img.shields.io/badge/Vercel-000000?style=for-the-badge&logo=vercel&logoColor=white" />
  <img width="8" />
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
</div>

---

## 🚀 **Estado de Despliegue**

| Componente | Plataforma | Estado | URL |
|------------|------------|--------|-----|
| **App Frontend** | Vercel | ✅ Activo | Installable PWA |
| **Modelos ML** | Embebidos (Joblib) | ✅ Cargados | V7 Specialized |
| **IA Estratégica** | Groq LLaMA 3.1 | ✅ Conectado | Latencia < 2s |

---

## 🏪 **Características Principales**

### **🔍 Análisis Clínico Preciso**
- Evaluación de Pulso, PCV, Nivel de Dolor y Proteína Total.
- Mapeo de 19 indicadores clínicos especializados.
- Soporte para diferentes tipos de lesiones y estados de membranas mucosas.

### **📱 Experiencia Mobile-First**
- Diseño PWA (Progressive Web App) instalable en iOS y Android.
- Interfaz premium con modo oscuro e interacciones fluidas.
- Iconografía médica profesional.

### **🧪 Rigor Científico**
- Modelado basado en el dataset de Colic de UCI.
- Entrenamiento con enfoque en Precisión Quirúrgica.
- Informes generados con terminología técnica veterinaria.

👨‍💻 **Desarrollador**
<div align="center">
**Santiago Arbelaez Contreras**

Junior Full Stack Developer 

Estudiante de Ingeniería de Sistemas – Universidad del Quindío

**Federico Alvarez**

Estudiante de Ingeniería de Sistemas – Universidad del Quindío

<br> <a href="https://github.com/santiagoarbelaezc"> <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" /> </a> <img width="10" /> <a href="https://www.linkedin.com/in/santiago-arbelaez-contreras-9830b5290/"> <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" /> </a> <img width="10" /> <a href="https://portfolio-santiagoa.web.app/portfolio"> <img src="https://img.shields.io/badge/Portfolio-6C63FF?style=for-the-badge&logo=sparkles&logoColor=white" /> </a></div>

<div align="center"> 
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=90&section=footer&animation=fadeIn" /> 
</div>
